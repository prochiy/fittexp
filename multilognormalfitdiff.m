...
lnd=@lognormaldiff; %переобозначение функции, чтобы строки были короче
for ii=1:4 %проверяем поля ставим флаги и определяем различные аргументы
    %борьба с глюками
...
%             ||isempty(str2num(cell2mat(get(eval(['handles.edDiff' num2str(ii)]),'String'))))
    if temp1 || temp2 || temp3    
        ...
        disp(['строка ' num2str(ii) ' не задействована'])
        term{ii}=' ';
    else
        DiffNMR.temp.state(ii)=true;
        disp(['строка ' num2str(ii) ' задействована'])
        %определяем один из членов функции мултилогнормального распределения
        DiffNMR.temp.term{ii}=['+a' mat2str(ii) '*lnd(g' num2str(ii) ',m' num2str(ii) ',s' num2str(ii) ',1)'];
        %определяем аргументы функции мультилогнормального распределения
        DiffNMR.temp.arg{ii}=[',a' mat2str(ii) ',g' num2str(ii) ',m' num2str(ii) ',s' num2str(ii)];
        %считывание флагов, отвечающих за то какой из параметров фиксировать
        ...
        %определение векторов согласно котором будет производится подгонка
        ...
        if get(handles.rbHook,'Value')==1
            %Определение векторов по котором будет производиться подгонка
            ...
            lvPop(ii)=length(DiffNMR.temp.vPop{ii}); %длинна векторов
            lvSig(ii)=length(DiffNMR.temp.vSig{ii});
            lvCD(ii)=length(DiffNMR.temp.vCD{ii});

            ciPop(ii)=find(DiffNMR.temp.vPop{ii}>pop(ii),1,'first'); %стартовые (текущие) индексы
...
%определяем функция мултилогнормального распределения
DiffNMR.temp.arg{1}(1)=[]; %удаляем первую запятую
DiffNMR.temp.arg{:};
eval(['mlnd=@(' DiffNMR.temp.arg{:} ') ' DiffNMR.temp.term{:} ';'])

if get(handles.rbNelder,'Value')==1
    multilognormaldiff %в этом скрипте реализован метод Нелдера-Мида
    return;
end

for ii=1:100
    cVal=sse(ciPop,ciCD,ciSig); %текущие значение среднеквадратичного отклонения
    for kk=1:4
        %disp('Итерации пошли')
        if DiffNMR.temp.state(kk)
            if rbPop(kk)~=1
                %шаг по параметру Pop
    ...
    %изменение текущего индекса
        if any(dPop) || any(dCD) || any(dSig)
            %disp('Изменение текущих индексов')
            ciPop=ciPop+dPop;
            ciCD=ciCD+dCD;
            ciSig=ciSig+dSig;           
        else
            disp('Подгонка закончилось')
             break;
        end
end

%вывод подгоночных значений
expr=' ';
expr2=' ';
for kk=1:4
    if DiffNMR.temp.state(kk)
...
expr2(end)=[]; %удаляем последнию не нужную запятую
...
%ниже следующая субфункция расчитывает среднеквадратичное отклонение
%экспериментальных точек от подгоночной кривой
...
arSimbol(end)=[]; %удаляем поледнию не нужную запятую
