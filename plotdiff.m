...
switch flag 
    case 'first'
        ...
        if eq(handles.first,1) %handles.first устанавливается равным 1 в функции mOpenFile_Callback(hObject, eventdata, handles)
            axis auto;
            % записываются начальные границы осей, для функции zoomdiff
        ...
        guidata(hObject,handles); % сохраняет структуру handles
    case 'down'
        DiffNMR.Click=1;
    ...
    case 'up'
        ...
        elseif handles.LineShift==1
            ...
            handles.InitialFirstPoint=DiffNMR.FirstPoint;
            handles.InitialSecondPoint=DiffNMR.SecondPoint;
            guidata(hObject,handles) %а вот здесь глюк иногда не происходит сохранения структуры
        end
    ...
    case 'plot'
        ...
        switch DiffNMR.Step % этот переключатель выводит данные в окна данных в соответствии
            case 1 % номером шага в цикле обработки
        ...
        %расчитанной экспонентой по данными введенным с мышки
    ...
    case 'firstpoint' %Если нажали на первую точку подгоночной линии
        handles=guidata(DiffNMR.MainAxes);
        DiffNMR.Click=1;
        handles.FirstPointShift=1;
        guidata(DiffNMR.MainAxes,handles)
   
    case 'secondpoint' %Если нажали на вторую точку подгоночной линии
        handles=guidata(DiffNMR.MainAxes);
        DiffNMR.Click=1;
        handles.SecondPointShift=1;
        guidata(DiffNMR.MainAxes,handles)
    
    case 'line' %Если нажила на саму подгоночную линию
        handles=guidata(DiffNMR.MainAxes);
        DiffNMR.Click=1;
        handles.LineShift=1;
        guidata(DiffNMR.MainAxes,handles)
